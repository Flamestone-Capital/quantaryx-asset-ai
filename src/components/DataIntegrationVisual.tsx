...
const DataIntegrationVisual = () => {
  ...
  const sourceDocuments = [
    { icon: <FileText className="text-blue-500 dark:text-blue-400" />, name: "dataIntegration.unstructured.documents.financialReport", rotation: -5 },
    { icon: <FileImage className="text-yellow-500 dark:text-yellow-400" />, name: "dataIntegration.unstructured.documents.invoiceScan", rotation: 3 },
    { icon: <FileSpreadsheet className="text-green-500 dark:text-green-400" />, name: "dataIntegration.unstructured.documents.dataTable", rotation: -2 },
    { icon: <FileBarChart className="text-purple-500 dark:text-purple-400" />, name: "dataIntegration.unstructured.documents.analysisChart", rotation: 4 },
    { icon: <Receipt className="text-orange-500 dark:text-orange-400" />, name: "dataIntegration.unstructured.documents.receiptImage", rotation: -3 },
    { icon: <FileType className="text-red-500 dark:text-red-400" />, name: "dataIntegration.unstructured.documents.handwrittenNote", rotation: 6 }
  ];

  // Table data for the structured result
  const tableData = [
    { account: "dataIntegration.structured.accounts.investmentA", value: "$1,245,000", change: "+2.3%" },
    { account: "dataIntegration.structured.accounts.retirementPlan", value: "$845,600", change: "+1.7%" },
    { account: "dataIntegration.structured.accounts.realEstate", value: "$1,650,000", change: "+0.5%" },
    { account: "dataIntegration.structured.accounts.cashReserve", value: "$320,000", change: "-0.2%" },
    { account: "dataIntegration.structured.accounts.stockInvestment", value: "$780,500", change: "+3.1%" },
    { account: "dataIntegration.structured.accounts.bondPortfolio", value: "$560,200", change: "+0.8%" },
    { account: "dataIntegration.structured.accounts.foreignCurrency", value: "$215,300", change: "-1.2%" },
    { account: "dataIntegration.structured.accounts.privateFund", value: "$450,000", change: "+4.5%" }
  ];

  // Chart data for the pie chart
  const pieChartSegments = [
    { color: "#9b87f5", percent: 35, label: "hero.charts.assetTypes.stocks" },
    { color: "#33C3F0", percent: 25, label: "hero.charts.assetTypes.bonds" },
    { color: "#FF8042", percent: 20, label: "hero.charts.assetTypes.realEstate" },
    { color: "#7E69AB", percent: 15, label: "hero.charts.assetTypes.alternative" },
    { color: "#1EAEDB", percent: 5, label: "hero.charts.assetTypes.cash" }
  ];
  ...
  const renderUnstructuredData = () => (
    ...
    >
      <h3 className="text-slate-800 dark:text-white text-lg font-semibold mb-2">{t('dataIntegration.unstructured.title')}</h3>
      <p className="text-slate-600 dark:text-gray-300 text-xs mb-3 text-center max-w-md">
        {t('dataIntegration.unstructured.description')}
      </p>
      ...
      <div className="grid grid-cols-3 gap-3 px-2">
        {sourceDocuments.map((doc, index) => (
          ...
          >
            ...
            <span className="text-xs text-slate-600 dark:text-gray-300 truncate w-full text-center">{t(doc.name)}</span>
          </motion.div>
        ))}
      </div>
    </motion.div>
  ...
  const renderAIProcessing = () => (
    <motion.div
      ...
      transition={{ duration: 0.3, ease: "easeOut" }}
    >
      <h3 className="text-slate-800 dark:text-white text-xl font-semibold mb-4">{t('dataIntegration.processing.title')}</h3>
      <p className="text-slate-600 dark:text-gray-300 text-sm mb-6 text-center max-w-md">
        {t('dataIntegration.processing.description')}
      </p>
  ...
  const renderStructuredData = () => (
    <motion.div
      ...
      transition={{ duration: 0.3, ease: "easeOut" }}
    >
      <h3 className="text-slate-800 dark:text-white text-xl font-semibold mb-4">{t('dataIntegration.structured.title')}</h3>
      <p className="text-slate-600 dark:text-gray-300 text-sm mb-6 text-center max-w-md">
        {t('dataIntegration.structured.description')}
      </p>
      ...
      <div className="space-y-6 w-full max-w-md">
        ...
        >
          ...
          <div className="bg-slate-100 dark:bg-gray-700/60 px-4 py-3 flex justify-between items-center border-b border-slate-200 dark:border-gray-600">
            <div className="flex items-center">
              ...
              <span className="text-slate-800 dark:text-white font-medium">{t('dataIntegration.structured.financialReport')}</span>
            </div>
            <span className="text-xs text-slate-600 dark:text-gray-300">{t('dataIntegration.structured.autoGenerated')}</span>
          </div>
          ...
          <div className="overflow-y-auto max-h-[200px] custom-scrollbar">
            <table className="w-full text-sm">
              <thead className="bg-slate-100 dark:bg-gray-700/50 sticky top-0 z-10">
                <tr>
                  <th className="py-2 px-4 text-left text-slate-700 dark:text-gray-300 font-medium">{t('dataIntegration.structured.tableHeaders.account')}</th>
                  <th className="py-2 px-4 text-right text-slate-700 dark:text-gray-300 font-medium">{t('dataIntegration.structured.tableHeaders.value')}</th>
                  <th className="py-2 px-4 text-right text-slate-700 dark:text-gray-300 font-medium">{t('dataIntegration.structured.tableHeaders.change')}</th>
                </tr>
              </thead>
              <tbody>
                {tableData.slice(0, 4).map((row, index) => (
                  ...
                  >
                    <td className="py-2 px-4 text-slate-800 dark:text-white">{t(row.account)}</td>
                    <td className="py-2 px-4 text-right font-medium text-slate-800 dark:text-white" style={{ color: row.account === "dataIntegration.structured.accounts.realEstate" ? "#33C3F0" : undefined }}>{row.value}</td>
        ...
        >
          <div className="flex items-center mb-3">
            ...
            <span className="text-slate-800 dark:text-white font-medium">{t('dataIntegration.structured.assetDistribution')}</span>
          </div>
                ...
                {pieChartSegments.slice(0, 3).map((segment, index) => (
                  ...
                  >
                    <div className="flex items-center">
                      ...
                      <span className="text-slate-600 dark:text-gray-300">{t(segment.label)}</span>
                    </div>
  ...
  return (
    <div className="w-full h-full bg-white dark:bg-gradient-to-br dark:from-gray-900 dark:to-gray-800 rounded-lg overflow-hidden relative border border-slate-200 dark:border-0">
      ...
      <div className="relative w-full h-full p-3 flex flex-col">
        <h2 className="text-slate-800 dark:text-white text-center mb-2 font-semibold text-xl">{t('dataIntegration.title')}</h2>
        
        {/* Flow process indicators */}
        <div className="flex justify-center mb-3 text-xs text-slate-600 dark:text-gray-400">
          <div className="flex items-center">
            <span className={`px-2 py-1 rounded-md transition-all duration-200 ${currentStep === 1 ? 'bg-indigo-100 dark:bg-quantaryx-purple/20 text-slate-800 dark:text-white font-medium' : 'bg-slate-100 dark:bg-gray-800/60'}`}>{t('dataIntegration.unstructured.title')}</span>
            <motion.div 
              animate={{ x: [0, 2, 0] }}
            ...
            <span className={`px-2 py-1 rounded-md transition-all duration-200 ${currentStep === 2 ? 'bg-indigo-100 dark:bg-quantaryx-purple/20 text-slate-800 dark:text-white font-medium' : 'bg-slate-100 dark:bg-gray-800/60'}`}>{t('dataIntegration.processing.title')}</span>
            <motion.div 
              animate={{ x: [0, 2, 0] }}
              transition={{ repeat: Infinity, duration: 1.2 }}
              className="mx-1"
            ...
            <span className={`px-2 py-1 rounded-md transition-all duration-200 ${currentStep === 3 ? 'bg-indigo-100 dark:bg-quantaryx-purple/20 text-slate-800 dark:text-white font-medium' : 'bg-slate-100 dark:bg-gray-800/60'}`}>{t('dataIntegration.structured.title')}</span>
          </div>
        </div>
